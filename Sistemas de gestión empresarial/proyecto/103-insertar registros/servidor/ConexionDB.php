	...
	class conexionDB{																													// Creo una nueva clase
		  ...
		  public function insertaTabla($tabla, $valores) {
            ...
            public function contarFilasTablas() {
                ...
                if (!$result) {
                    return json_encode(['resultado' => 'error', 'mensaje' => mysqli_error($this->conexion)]);
                }
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    if (!isset($row['TABLE_NAME'])) {
                        continue; // Skip if TABLE_NAME is not set
                    }
                    ...
                    if (!$resultCount) {
                        return json_encode([
                            'resultado' => 'error', 
                            'mensaje' => "Error al contar filas en la tabla $tabla: " . mysqli_error($this->conexion)
                        ]);
                    }
